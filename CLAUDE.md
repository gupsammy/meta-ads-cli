...
| `intel fetch-daily` | Scriptable ad×daily metrics pull for an explicit date range (backfill / catch-up) | `--since <YYYY-MM-DD>` and `--until <YYYY-MM-DD>` (both required, `since ≤ until`), `--access-token`. Writes `ads-daily.json` to `<dataDir>/daily/<since>_<until>/`. No `effective_status` filter (returns every ad that delivered in the window); fetches all rows (no cap). `--keep-video` additionally snapshots the account's **current** ad creatives and retains each source video to the shared `~/.meta-ads-intel/creatives/<ad_id>/video.mp4` (reuses `intel run --keep-video`'s pipeline). Window-independent (Meta video URLs expire for deleted ads); best-effort — ffmpeg-absent or zero-creatives skips the video step without failing the metrics pull. Snapshot, not archive: the shared creatives dir is atomically **replaced** each run (same as `intel run --keep-video`), so tag videos promptly — the durable artifact is the SQLite tag, not the file. Creative fetch is capped at 500 ads; a truncation notice surfaces in `result.creatives.warnings`. Takes the same `<dataDir>/.pull-lock` as `intel run` (exits with the lock error if a pull is in progress; stale >30 min auto-cleared) |
